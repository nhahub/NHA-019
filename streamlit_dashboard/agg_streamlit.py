...
# Custom CSS for better styling + DARK GREEN SIDEBAR
st.markdown("""
<style>
    .stApp {
    ...
    [data-testid="stSidebar"] {
        background-color: #084635 !important;  /* Dark green sidebar */
    }
    [data-testid="stSidebar"] * {
        color: #F0FDF4 !important;  /* Light text */
    }
    [data-testid="stSidebar"] .css-1d391kg,
    [data-testid="stSidebar"] .css-1cpxqw2,
    [data-testid="stSidebar"] .css-1lcbmhc,
    [data-testid="stSidebar"] .css-1outpf7,
    ...
    /* Checkbox & Slider track */
    [data-testid="stSidebar"] .stCheckbox > div > div,
    [data-testid="stSidebar"] .stSlider .css-1cpxqw2 {
        background-color: #047857 !important;
    }
</style>
...
def load_all_data():
    """Load only the latest 2000 sensor records from database (fast & efficient)"""
    try:
        ...
        query = """
        SELECT 
            farm_id, region, temperature, soil_moisture, humidity,
            sunlight_intensity, soil_ph, pesticide_usage_ml, rainfall, timestamp
        FROM sensor_data
        ORDER BY timestamp DESC
        LIMIT 2000;                     -- This line gets only the last 2000 rows
        """
        
        df = pd.read_sql(query, conn)
        conn.close()
        
        st.sidebar.success(f"Loaded {len(df):,} recent records (Last 2000)")
        return df
...
def display_current_readings(latest, previous, total_pesticide):
    ...
    with col1:
        ...
        color = "#38A169" if delta_moisture > 0 else "#E53E3E" if delta_moisture < 0 else "#718096"
        st.markdown(f"""
    ...
    with col2:
        ...
        color = "#E53E3E" if delta_temp > 0 else "#38A169" if delta_temp < 0 else "#718096"
        st.markdown(f"""
        <div class="metric-box">
    ...
    with col3:
        ...
        <div class="metric-box">
            ...
            <div class="metric-delta" style="color: #38A169;">+0.0 mm</div>
        </div>
        """, unsafe_allow_html=True)
...
def create_trend_charts(filtered_df):
    ...
    fig_moisture.add_hline(y=40, line_dash="dash", line_color="#DD6B20", annotation_text="Warning Threshold (40%)")
    fig_moisture.add_hline(y=30, line_dash="dash", line_color="#E53E3E", annotation_text="Critical Threshold (30%)")
    fig_moisture.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
    st.plotly_chart(fig_moisture, use_container_width=True)
    
    col1, col2 = st.columns(2)
    
    with col1:
        fig_temp = px.line(filtered_df, x="timestamp", y="temperature", title="Temperature Monitoring", labels={"temperature": "Temperature (°C)"}, color_discrete_sequence=["#065F46"])
        fig_temp.add_hline(y=35, line_dash="dash", line_color="#E53E3E", annotation_text="High Temp Alert (>35°C)")
        fig_temp.add_hline(y=10, line_dash="dash", line_color="#3182CE", annotation_text="Low Temp Alert (<10°C)")
        fig_temp.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
        st.plotly_chart(fig_temp, use_container_width=True)
        
        fig_ph = px.line(filtered_df, x="timestamp", y="soil_ph", title="Soil pH Monitoring", labels={"soil_ph": "Soil pH"}, color_discrete_sequence=["#065F46"])
        fig_ph.add_hline(y=6, line_dash="dash", line_color="#E53E3E", annotation_text="Low pH Alert (<6)")
        fig_ph.add_hline(y=7.5, line_dash="dash", line_color="#DD6B20", annotation_text="High pH Alert (>7.5)")
        fig_ph.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
        st.plotly_chart(fig_ph, use_container_width=True)
    
    with col2:
        fig_hum = px.line(filtered_df, x="timestamp", y="humidity", title="Humidity Monitoring", labels={"humidity": "Humidity (%)"}, color_discrete_sequence=["#065F46"])
        fig_hum.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
        st.plotly_chart(fig_hum, use_container_width=True)
        
        fig_sun = px.line(filtered_df, x="timestamp", y="sunlight_intensity", title="Sunlight Intensity Monitoring", labels={"sunlight_intensity": "Sunlight Intensity (W/m²)"}, color_discrete_sequence=["#065F46"])
        fig_sun.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
        st.plotly_chart(fig_sun, use_container_width=True)
    
    fig_rain = px.bar(filtered_df, x="timestamp", y="rainfall", title="Rainfall Monitoring", labels={"rainfall": "Rainfall (mm)"}, color_discrete_sequence=["#065F46"])
    fig_rain.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#065F46"))
    st.plotly_chart(fig_rain, use_container_width=True)
    
    fig_pest = px.line(filtered_df, x="timestamp", y="pesticide_usage_ml", title="Pesticide Usage Monitoring", labels={"pesticide_usage_ml": "Pesticide Usage (ml)"}, color_discrete_sequence=["#065F46"])
    fig_pest.update_layout(plot_bgcolor="#F0FDF4", paper_bgcolor="#235549", font=dict(color="#084635"))
...
def main():
    ...
    if filtered_df.empty:
        st.warning("No data for selected farm and region.")
        return
    ...
    table_data = table_data.loc[::-1].reset_index(drop=True)
